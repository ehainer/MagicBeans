					...
					if options[:data][:preserve] == true
						options[:data][:files] = [@model.send(method)].flatten.compact.map do |upload|
							if !upload.nil? && !upload.file.nil? && File.exist?(upload.file.path)
								{ Digest::SHA256.hexdigest("#{@model.class.name.underscore}_#{upload.file.try(:filename)}") => { url: upload.url, status: :added, accepted: true, name: upload.file.try(:filename), size: upload.file.try(:size), type: upload.file.try(:content_type) } }
							end
